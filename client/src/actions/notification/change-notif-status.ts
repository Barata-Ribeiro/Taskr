...
import { changeNotificationStatusUrl } from "@/helpers/backend-routes"
import { auth } from "auth"
import { revalidateTag } from "next/cache"

export default async function changeNotifStatus(id: number | string, isRead: boolean) {
    const session = await auth()

    try {
        if (!session) return ResponseError(unauthenticated)

        const URL = changeNotificationStatusUrl(id, isRead)
