...
import { notificationByIdUrl } from "@/helpers/backend-routes"
import { auth } from "auth"

export default async function getNotifById(id: number | string) {
    const session = await auth()

    try {
        if (!session) return ResponseError(unauthenticated)

        const URL = notificationByIdUrl(id)
