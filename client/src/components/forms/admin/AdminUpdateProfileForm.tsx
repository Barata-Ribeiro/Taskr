...
import adminUpdateUserByUsername from "@/actions/admin/admin-update-user-by-username"
...
interface AdminUpdateProfileFormProps {
    profilePromise: ReturnType<typeof adminGetUserByUsername>
}

export default function AdminUpdateProfileForm({ profilePromise }: Readonly<AdminUpdateProfileFormProps>) {
    ...
    const [formState, formAction, pending] = useActionState(adminUpdateUserByUsername, applicationInitialState())
    ...
    }, [formState.ok, formState.response?.data])
    ...
    return (
        <form action={formAction} className="md:col-span-2">
            <div className="grid grid-cols-1 gap-x-6 gap-y-8 sm:max-w-xl sm:grid-cols-6">
                <input type="hidden" name="currentUsername" defaultValue={initialProfile.username} readOnly />
                ...
                <div className="col-span-full">
                    <DefaultMarkdownEditor
                        label="Biography"
                        name="bio"
                        placeholder="Write a biography for this user..."
                        height={200}
                        maxLength={200}
                        value={bodyContent}
