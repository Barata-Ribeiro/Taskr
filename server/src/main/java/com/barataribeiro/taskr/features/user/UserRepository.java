...
public interface UserRepository extends JpaRepository<User, UUID>, RepositorySpecificationExecutor<User, UUID> {
    ...
    @Query("""
           select new com.barataribeiro.taskr.features.user.dtos.UserSearchDTO(u.id, u.username, u.createdAt) from User u
    ...
    @Query("""
           select new com.barataribeiro.taskr.features.stats.dtos.UserStatsDTO(
                count(p),
                count(t),
                count(c),
